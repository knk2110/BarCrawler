...
templates.mainPageStructure = [
	...
	'		<h1>BarCrawler</h1>',
	'		<h4>Create and manage your bar crawls</h4>',
	'	</div>',
	'	</div>',
	'	<div id="pageContent" class="col-md-8">',
	'		<P></P>',
	'		<P>',
	'			<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#newCrawlModal"><span class="glyphicon glyphicon-plus"></span> Create New Crawl</button>',
	'		</P>',
	'	</div>',
	...
	'									<input type="text" required class="form-control" id="newCrawlFormTitle" name="title" size="5" placeholder="Enter a name" autofocus>',
	'								</div>',
	'								<div class="form-group">',
	'									<label for="newCrawlFormDate">Date</label>',
	'									<input type="date" required class="form-control" id="newCrawlFormDate" name="date">',
	'								</div>',
	'								<div class="form-group">',
...
templates.mainPageCrawlPanel = [
	...
	'	<div class="panel-heading">',
	'		<button type="button" class="btn btn-xs btn-default" data-toggle="collapse" data-target="#{{= id =}}-crawl-panel-content"><span class="glyphicon glyphicon-list"></span></button>',
...
templates.mainPageCrawlPanelBar = [
	...
	'				<strong>{{= name =}}</strong> ({{= category =}}) {{ if (rating != "") { }} <span class="label label-info">{{= rating =}} / 10</span> {{ } }} <span class="label label-success">{{= rept("$",price) =}}</span> {{ if (typeof url != "undefined") { }} <a class="btn btn-xs btn-default" href="{{= url =}}" target="_blank"><span class="glyphicon glyphicon-info-sign"></span></a>{{ } }}</span>',
...
templates.editPageStructure = [
  	'	<p>',
	'		<button type="button" class="btn btn-primary" id="home"><span class="glyphicon glyphicon-home"></span> Home</button>',
	'		<button type="button" class="btn btn-primary saveCrawl"><span class="glyphicon glyphicon-home"></span> Home</button>',
	'	</p>',
	'	<p>',
	'		<div class="row">',
	'			<div class="col-md-4">',
	...
	'									<input type="text" required class="form-control" id="barSearchFormZip" name="zip" size="5" maxlength="5" pattern="[0-9][0-9][0-9][0-9][0-9]" placeholder="zip">',
	'								</div>',
	'								<div class="form-group">',
	...
	'			</div>',
	'			<div class="col-md-7">',
	'				<div class="panel panel-default" id="crawlPanel">',
	'					<div class="panel-heading">',
	...
	'								</div>',
	'								<button type="button" class="btn btn-primary saveCrawl" id="crawlFormSave"><span class="glyphicon glyphicon-floppy-disk"></span> Save</button>',
...
// Creates a drop down list for sorting the search results
templates.editPageSearchSort = [
	...
	'					{{ if (num>0) { }} <option value="distance">Distance from ({{= num =}})</option>{{ } }}',
	'					<option id="priceAsc" value="priceAsc">Price (low-to-high)</option>',
	'					<option id="priceDesc" value="priceDesc">Price (high-to-low)</option>',
	'					<option id="ratingAsc" value="ratingAsc">Rating (low-to-high)</option>',
	'					<option id="ratingDesc" value="ratingDesc">Rating (high-to-low)</option>',
	'				</select>',
	'			</div>',
	'			</div>',
	'	</form>',
...
templates.editPageSearchPanelBar = [
	...
	'				<span class="text-muted"><br><span class="text-muted">{{ if (typeof location.address != "undefined") { }} {{= location.address =}}, {{ } }} {{ if (typeof location.city != "undefined") { }} {{= location.city =}}, {{ } }} {{= location.state =}} {{= location.postalCode =}}</span>',
